...
int main(int, char *[]) {
  ...
  float* params = (float*)malloc(sizeof(float) * MyLayer2::parameters_no);
  float* inputs = (float*)malloc(sizeof(float) * IN_MAPS * IN_HEIGHT * IN_WIDTH);
  //float* storage = (float*)malloc(sizeof(float) * MyLayer2::internal_no);
  //float* outputs = (float*)malloc(sizeof(float) * MyLayer::outputs_no);
  float* outputs2 = (float*)malloc(sizeof(float) * MyLayer2::outputs_no);
  ...
  uniform_real_distribution<float> next_float(-0.05, 0.05);

  // Init inputs
  for (size_t i = 0; i < IN_MAPS; i++) {
    for (size_t r = 0; r < IN_HEIGHT; r++) {
      for (size_t c = 0; c < IN_WIDTH; c++) {
        inputs[i * (IN_HEIGHT * IN_WIDTH) + r * IN_WIDTH + c] = next_float(e);
      }
    }
  }
  /*
    ...
    for (size_t i = 0; i < IN_MAPS; i++) {
      for (size_t r = 0; r < CONV_HEIGHT; r++) {
        for (size_t c = 0; c < CONV_WIDTH; c++) {
          params[MyLayer2::p_idx(j, i, r, c)] = next_float(e);
        }
      }
    }
  }
