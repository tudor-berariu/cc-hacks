...
int main(int, char *[]) {
  ...
  double* params = (double*)malloc(sizeof(double) * MyLayer::parameters_no);
  double* inputs = (double*)malloc(sizeof(double) * IN_MAPS * IN_HEIGHT * IN_WIDTH);
  //double* storage = (double*)malloc(sizeof(double) * MyLayer::internal_no);
  ...
  cout << "Am facut un mare layer cu "
       << MyLayer::inputs_no << " intrari, "
       << MyLayer::parameters_no << " parametri, "
       << MyLayer::out_height << "x" << MyLayer::out_width << " = "
       << MyLayer::outputs_no << " iesiri" << endl;
    ...
    for (size_t i = 0; i < IN_MAPS; i++) {
      for (size_t r = 0; r < CONV_HEIGHT; r++) {
        for (size_t c = 0; c < CONV_WIDTH; c++) {
          params[MyLayer::p_idx(j, i, r, c)] = next_double(e);
        }
      }
    }
  }
  ...
  */
  ...
  //MyLayer2::forward(inputs, storage, params, outputs2);
  /*
  for (size_t j = 0; j < OUT_MAPS; j++) {
    for (size_t r = 0; r < MyLayer::out_height; r++) {
      for (size_t c = 0; c < MyLayer::out_width; c++) {
        cout.width(8);
        cout.precision(6);
        cout << outputs2[j * (MyLayer::out_height * MyLayer::out_width) +
                         r * MyLayer::out_width + c] << " ";
      }
      cout << endl;
    }
  ...
  */
  ...
  MyLayer::forward(inputs, params, outputs);
  ...
  for (size_t j = 0; j < OUT_MAPS; j++) {
    for (size_t r = 0; r < MyLayer::out_height; r++) {
      for (size_t c = 0; c < MyLayer::out_width; c++) {
        x += ((j % 2) - 0.5) * outputs[j *
                                       (MyLayer::out_height * MyLayer::out_width) +
                                       r * MyLayer::out_width + c];
      }
    }
  }
